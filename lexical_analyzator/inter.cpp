...
void Parser::D1 (vector<Ident>& tid, bool flagIsStruct)  // данные в структуре
{ 
...
void Parser::D (TypeOfLex type, vector<Ident>& tid, bool flagIsStruct) { // описание переменных
  if (currType_ != LEX_ID) {
    throw currLex_;
  }
  else {
...
void Parser::B1 (bool isCycle) // штука для break (чтоб только из циклов выходили)
{
...
void Parser::B (bool isCycle) // тут все самое крутое
{
  int pl0, pl1, pl2, pl3;
  int l1, l2, l3, l4;
  ///////////////////////////////////////////////////////
...
void Parser::E (Lex currLex, bool notObject, bool isWrite) // выражения обрабатываем
{ 
  int i,j;
  if (currType_ == LEX_ID) {
    gl();
...
void Parser::S (Lex currLex) // присваивания
{
  if (currType_ == LEX_ID) {
    checkObject();
    gl();
...
void Parser::checkPlus() 
{
  if (stLex_.top() != LEX_INT) {
    throw "wrong type is in MINUS";
  }
  else {
    poliz.push_back (Lex (POLIZ_PLUS));
  }
}
